...
<script>
	...
	<h2 style="margin:50px; width:100%; text-align:center;">파워 목록</h2>
	<!-- 리스트 목록 검색 -->
	<div class="container" style="margin-top:30px; margin-bottom:30px;">
		<form name="search_list" method="post" action="<%= request.getContextPath() %>/power/listPost">
			...
			<table class="table table-bordered">
				<tbody>
					<tr>
						<td width="20%">제조사</td>
						<td width="80%">
							<label class="checkbox-inline"><input type="checkbox" name="productCompany" value="MICRONICS" <c:if test="${fn:contains(line, 'MICRONICS')}">checked</c:if>>MICRONICS</label>
							<label class="checkbox-inline"><input type="checkbox" name="productCompany" value="ANTEC" <c:if test="${fn:contains(line, 'ANTEC')}">checked</c:if>>ANTEC</label>
							<label class="checkbox-inline"><input type="checkbox" name="productCompany" value="ZALMAN" <c:if test="${fn:contains(line, 'ZALMAN')}">checked</c:if>>ZALMAN</label>
							<script>
