...
<script>	
	...
	function sortItem(orderby, sql) {
		...
		$.ajax({
			url:"/final_project/graphic/prolist?orderby="+orderby+"&sql="+encodeURI(sql),
			type: "GET"
		}).done(function(data){
...
<script>
	...
	<h2 style="margin:50px; width:100%; text-align:center;">그래픽카드 목록</h2>
	<!-- 리스트 목록 검색 -->
	<div class="container" style="margin-top:30px; margin-bottom:30px;">
		<form name="search_list" method="post" action="<%= request.getContextPath() %>/graphic/listPost">
			...
			<table class="table table-bordered">
				<tbody>
					<tr>
						<td width="20%">제조사</td>
						<td width="80%">
							<label class="checkbox-inline"><input type="checkbox" name="productCompany" value="WD" <c:if test="${fn:contains(line, 'WD')}">checked</c:if>>WD</label>
							<label class="checkbox-inline"><input type="checkbox" name="productCompany" value="SEAGATE" <c:if test="${fn:contains(line, 'SEAGATE')}">checked</c:if>>SEAGATE</label>
							<label class="checkbox-inline"><input type="checkbox" name="productCompany" value="TOSHIBA" <c:if test="${fn:contains(line, 'TOSHIBA')}">checked</c:if>>TOSHIBA</label>
							<script>
					...
					<tr>
						<td width="20%">디스크 검색</td>
						<td width="80%"><input type="text" name="diskSize" value="${diskSize}" size="100" maxlength="50"></td>
					</tr>
					<tr>
