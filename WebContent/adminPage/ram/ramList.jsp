    ...
    <h1 style="text-align:center;">RAM 관리</h1>     
      ...
      <div class="w3-container">
      	<div style="text-align:right;">
			...
			<span class="w3-right"><input type="button" class="btn btn-default" value="Create" onclick="window.location='<%= request.getContextPath() %>/admin/ram/write'"></span>
		</div>
			...
			<c:if test="${ramList != null}">
				...
				<tr>
					...
					<td align="center" width="150">
			        	<a href="<%= request.getContextPath() %>/admin/ram/update?id=${ ram.id }" class="btn btn-sm btn-default" data-toggle="tooltip" data-placement="top" title="수정">
			            	<em class="fa fa-pencil"></em>
			        	</a>
			        	<a href="<%= request.getContextPath() %>/admin/ram/detail?id=${ ram.id }" class="btn btn-sm btn-default" data-toggle="tooltip" data-placement="top" title="상세보기 ">
		...
		<div class="w3-center">
			<c:if test="${startPage > bottomLine }">
				<a href="<%= request.getContextPath() %>/admin/ram/list?pageNum=${  startPage - bottomLine }">[이전]</a>
			</c:if>
		
			<ul class="pagination">
			<c:forEach var="i" begin="${startPage}" end="${endPage}">
				<li>
					<a href="<%= request.getContextPath() %>/admin/ram/list?pageNum=${ i }">
			...
			<c:if test="${endPage < pageCount }">
				<a href="<%= request.getContextPath() %>/admin/ram/list?pageNum=${ startPage + bottomLine }">[다음]</a>
			</c:if>
		</div>
