...
%>
   ...
   <a href="<%=request.getContextPath() %>/board2/write">글쓰기</a></span>
   ...
   <table class="w3-table-all" width="700">
    ...
    <c:if test="${article.re_level > 0 }"> 
         ...
         <a href="<%=request.getContextPath() %>/board2/content?num=${article.num }&no=${number}">${article.subject }</a></td>         
         ...
         <td align="center" width="100">${article.ip }</td>
            ...
            <c:if test="${startPage > bottomLine }">
                  <a href="<%=request.getContextPath() %>/board2/list?pageNum=${startPage - bottomLine}">[이전]</a>
            </c:if>
            
            <c:forEach var="i" begin="${startPage }" end="${endPage }">
               <a href="<%=request.getContextPath() %>/board2/list?pageNum=${i}">
            ...
            <c:if test="${endPage < pageCount }">
                  <a href="<%=request.getContextPath() %>/board2/list?pageNum=${endPage+1}">[다음]</a>
            </c:if>
            </div>
   </c:if>
   </div>
